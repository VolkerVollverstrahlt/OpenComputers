...
object BlockPosition {
  def apply(x: Int, y: Int, z: Int, level: Level) = new BlockPosition(x, y, z, Option(level))

  def apply(x: Int, y: Int, z: Int) = new BlockPosition(x, y, z, None)

  def apply(x: Double, y: Double, z: Double, level: Level) = new BlockPosition(x, y, z, Option(level))

  def apply(x: Double, y: Double, z: Double) = new BlockPosition(x, y, z, None)

  def apply(v: Vector3d) = new BlockPosition(v.x, v.y, v.z, None)

  def apply(v: Vector3d, level: Level) = new BlockPosition(v.x, v.y, v.z, Option(level))
  ...
  def apply(pos: BlockPos, level: Level): BlockPosition = BlockPosition(pos.getX, pos.getY, pos.getZ, level)
